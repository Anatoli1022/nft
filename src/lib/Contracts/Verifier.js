...
export class Verifier {
    ...
    async getApprowedNftCollection() {
        let verifierContract = new Contract(VERIFIER_CONTRACT_ADDRESS, VERIFIER_CONTRACT_ABI, this.provider);
        ...
        events.forEach(event => {
            console.log(event)
            let ifacelog = iface.decodeEventLog('ApprowedNftCollection', event.data, event.topics);
            resultEvents.push({
    ...
    async setName(_name) {
        let verifierContract = new Contract(VERIFIER_CONTRACT_ADDRESS, VERIFIER_CONTRACT_ABI, this.signer);
